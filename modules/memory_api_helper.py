...
# === IMPORT MODULE BỘ NHỚ CHUNG ===
from modules.sheets import (
    save_memory, get_memory, search_memory,
    clear_memory
)
from modules.memory_api_helper import fetch_memory_from_api, save_memory_to_api  # NEW
...
def get_ai_response(user_prompt, user_id=None):
    ...
    memory_context = ""
    if user_id:
        mems = fetch_memory_from_api(user_id)  # NEW: Gọi từ API Zeabur
        if mems:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state.get("awaiting_note"):
        ...
        save_memory_to_api(user_id, text, note_type)  # NEW – đồng bộ qua API
